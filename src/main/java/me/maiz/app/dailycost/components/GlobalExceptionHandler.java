...
public class GlobalExceptionHandler {
    ...
    public String processAppException(AppException e ,HttpServletRequest request){
        ...
        if(e.getResultCode()== DailyCostResultCode.INVALID_ARGUMENT){
            ...
            //使用get方法访问页面地址
            return redirectToReqeustURI(requestURI,request,"参数不合法（" + e.getMessage() + ")");
        }

        if(e.getResultCode()== DailyCostResultCode.USER_NOT_FOUND_OR_PASSWORD_ERROR){
            request.setAttribute(Constants.MSG_KEY,DailyCostResultCode.USER_NOT_FOUND_OR_PASSWORD_ERROR.getMessage());
            return "forward:/index.jsp";
        }
        return DEFAULT_ERROR_PAGE;
    }

    private String redirectToReqeustURI(String requestURI,HttpServletRequest request,String msg) {
        try {
            return "redirect:" + requestURI + "?" + Constants.MSG_KEY + "=" + URLEncoder.encode(msg, "UTF-8");
